...
namespace Mediapipe.Unity.Sample.Holistic
{
  ...
  {
    [SerializeField] private SignPredictionProvider predictionProvider;
    ...
    protected override IEnumerator Run()
    {
      ...
      while (true)
      {
        ...
        if (runningMode.IsSynchronous())
        {
          ...
          {
              var leftHand = ConvertLandmarks(result.leftHandLandmarks, 21);
              var rightHand = ConvertLandmarks(result.rightHandLandmarks, 21);
              var face5 = ExtractFace5(result.faceLandmarks);

              /*
              Debug.Log(
                  $"[HolisticBridge] face={(face5 != null ? face5.Length : 0)}, " +
                  $"left={(leftHand != null ? leftHand.Length : 0)}, " +
                  $"right={(rightHand != null ? rightHand.Length : 0)}"
              );
              */

              predictionProvider.AddLandmarkFrame(leftHand, rightHand, face5);
          }
    ...
    // landmark -> vector3 변환 코드 추가
    private Vector3[] ConvertLandmarks(NormalizedLandmarkList landmarks, int expectedCount)
    {
        if (landmarks == null || landmarks.Landmark.Count == 0)
            return null;

        int count = Mathf.Min(expectedCount, landmarks.Landmark.Count);
        Vector3[] result = new Vector3[count];

        for (int i = 0; i < count; i++)
        {
            var lm = landmarks.Landmark[i];
            result[i] = new Vector3(lm.X, lm.Y, lm.Z);
        }
    ...
    // 얼굴 포인트 중 5개 선택 함수
    private Vector3[] ExtractFace5(NormalizedLandmarkList face)
    {
        if (face == null || face.Landmark.Count < 300)
            return null;

        int[] idx = { 1, 33, 263, 61, 291 };

        Vector3[] result = new Vector3[5];

        for (int i = 0; i < 5; i++)
        {
            var lm = face.Landmark[idx[i]];
            result[i] = new Vector3(lm.X, lm.Y, lm.Z);
        }
